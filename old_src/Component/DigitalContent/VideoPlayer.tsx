...
const VideoPlayer:React.FC<{videos:any}> = ({ videos }) => {
  ...
  const sortedVideos = videos?.slice().sort((a:any, b:any) => {
    const extractNumericValue = (str:string) => {
      const match = str.match(/\d+/);
      return match ? parseInt(match[0]) : NaN;
    };
  
    const numericA = extractNumericValue(a.VideoName);
    const numericB = extractNumericValue(b.VideoName);
  
    if (!isNaN(numericA) && !isNaN(numericB)) {
      // If both titles have numbers, compare them numerically
      if (numericA !== numericB) {
        return numericA - numericB;
      }
    }
  
    // If numbers are equal or not present, compare the entire string case-insensitively
    return a.VideoName.localeCompare(b.VideoName, 'en', { sensitivity: 'base', numeric: true });
  });
