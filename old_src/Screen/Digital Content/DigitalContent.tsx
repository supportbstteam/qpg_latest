...
const DigitalContent = ({ route, navigation }) => {
  ...
  const handlePress = (screen:any,data:any) => {
    console.log(data)
    if(data === undefined || data.success===false || data.length === 0){
      Toast.show({
        type: 'error',
        ...
        <View style={{ width: '100%' }}>
          ...
          {userRole === 4 ? (
            <>
              <TouchableOpacity
                style={styles.btnn}
                onPress={()=>handlePress('Animations',videos)}>
                <Image
