...
const ConfirmOrderHistory = ({ navigation }) => {
    ...
    const filterOrdersByType = () => {
        return orderHistory.filter((order) => order.order_type === (activeTab === 'ownOrders' ? 1 : 2));
    };
    ...
    const handleApprove = async (OrderId) => {
        ...
        try {
            ...
            Toast.show({ type: 'success', text1: "Status Approved", visibilityTime: 2000 });
            getHistory();
        } catch (error) {
    ...
    const handleReject = async (OrderId) => {
        ...
        try {
            const response = await api.Order_Approve_reject(OrderId, token, formData);
            Toast.show({ type: 'success', text1: "Status Rejected", visibilityTime: 2000 });
            getHistory();
        } catch (error) {
        ...
        <>
            {item.order_type === 1 && (
                <TouchableOpacity
                    ...
                    onPress={() => navigation.navigate("ViewConfirmOrder", { orderId: item.id })}>
                    ...
                    {expandedUserId === item.id && (
                        <FlatList
                            ...
                            renderItem={({ item: orderItem }) => (
                                <TouchableOpacity
                                    style={styles.cardContainer}
                                    onPress={() => navigation.navigate("ViewConfirmOrder", { orderId: null, item: orderItem })}>
                                    <View style={styles.cardContent}>
                                        <Textlabel title="Order No:" value={orderItem?.id} />
                                        <Textlabel title="Date:" value={dateTimeFormat(orderItem.created_at)} />
                                        <Textlabel title="Total Amt:" value={orderItem?.total_amount} />
                                        <Textlabel title="Status:" value={getStatusText(orderItem.status)} />
                                        {orderItem.status === 0 && (
        ...
        <View style={styles.container}>
            ...
            <View style={{ flexDirection: "row", justifyContent: "space-between", marginVertical: 10, }}>
                ...
                <TouchableOpacity style={{ marginHorizontal: 20, borderWidth: 1, padding: 10, borderRadius: 10 }} onPress={() => { navigation.navigate("AddOrder") }}>
                    ...
                    <Text style={{ fontSize: responsiveFontSize(2), color: "#000" }}>Add Order</Text>
                </TouchableOpacity>
            ...
            <View style={styles.tabContainer}>
                ...
                >
                    <Text style={[styles.tabText, activeTab === 'ownOrders' && styles.activeTabText]}>
                        Own Orders
                    </Text>
                </TouchableOpacity>
                <TouchableOpacity
                    style={[styles.tab, activeTab === 'assignedOrders' && styles.activeTab]}
                    onPress={() => setActiveTab('assignedOrders')}
                >
                    <Text style={[styles.tabText, activeTab === 'assignedOrders' && styles.activeTabText]}>
                        Assigned Orders
                    </Text>
                </TouchableOpacity>
            </View>
